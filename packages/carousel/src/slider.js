...
class Slider extends PureComponent {
  ...
  constructor(props) {
    ...
    this.getSlideTo = this.getSlideTo.bind(this);
  }
  ...
  getSlideTo (slideIndex) {
    return () => {this.setState(Slider.slideTo(slideIndex))};
  }
  ...
  handleSwipeMove(position, event) {
    ...
    this.setState(Slider.slideTo(this.state.slideIndex, false, position.x));
      ...
      <div className="slider">
        ...
        <div className="bullets">
          {this.props.children.map((_, index) => {
            const isActive = (index === 0 && this.state.slideIndex === slides.length -1)
              || index + 1 === slideIndex
              || (index + 1 === this.props.children.length && this.state.slideIndex === 0)
            return (
              <span key={index} onClick={this.getSlideTo(index + 1)} className={`bullet ${isActive && 'bullet-active'}`}/>
            );
          })}
        </div>
        <style jsx>{`
